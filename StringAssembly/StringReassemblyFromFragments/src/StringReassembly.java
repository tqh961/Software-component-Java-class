...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //Return the combination which is str1[0, |str1|-overlap) * str2
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //Stop the recursion when the set is zero
        if (strSet.size() == 0) {
            //Add str into set
            strSet.add(str);
            ...
            //Remove an element from the set strSet
            String element = strSet.removeAny();

            //Check if element has the index of str
            ...
            //Check if str has the index for element
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> newSet = new Set2<>();

        //Loop stop when the input is not at the end of stream
        while (!input.atEOS()) {

            //Set new set
            //except that any line that is a substring of another not in the set
            addToSetAvoidingSubstrings(newSet, input.nextLine());
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //Set loop to go through each characters of the text
        int i = 0;
        while (i < text.length()) {

            //If it is ~, it will have line separator
            if (text.charAt(i) == '~') {

                out.println();

            } else {
                //Or else print the characters
