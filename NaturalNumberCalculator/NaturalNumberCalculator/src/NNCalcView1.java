...
public final class NNCalcView1 extends JFrame implements NNCalcView {
    ...
    public NNCalcView1() {
        ...
        for (int i = 0; i < DIGIT_BUTTONS; i++) {
            NaturalNumber integerString = new NaturalNumber1L(i);
            this.bDigits[i] = new JButton(integerString.toString());
        ...
        JPanel sideButton = new JPanel(new GridLayout(
        ...
        sideButton.add(this.bClear);
        sideButton.add(this.bSwap);
        sideButton.add(this.bEnter);
         ...
         */
        JPanel combinedButton = new JPanel(new FlowLayout());

        /*
         * Add the other two button panels to the combined button panel
         */
        combinedButton.add(buttonPanel);
        combinedButton.add(sideButton);
        ...
        for (int j = 0; j < DIGIT_BUTTONS; j++) {
            this.bDigits[j].addActionListener(this);
    ...
    public void registerObserver(NNCalcController controller) {
        ...
        //register controller to this.controller
    ...
    public void updateTopDisplay(NaturalNumber n) {
        ...
        //Updates top display text with Naturalnumber n
    ...
    public void updateBottomDisplay(NaturalNumber n) {
        ...
        //Update bottom display with Naturalnumber n
    ...
    public void updateSubtractAllowed(boolean allowed) {
        ...
        //setEnabled when subtract is allowed
        if (allowed) {
            this.bSubtract.setEnabled(true);

        } else {
            //SetEnabled false when subtract is not allowed
    ...
    public void updateDivideAllowed(boolean allowed) {
        ...
        //setEnabled when divide is allowed
        if (allowed) {
            this.bDivide.setEnabled(true);

        } else {
            //setEnabled to be false when it is ot allowed
            this.bDivide.setEnabled(false);
    ...
    public void updatePowerAllowed(boolean allowed) {
        ...
        //SetEnabled to be true, when it is allowed
        if (allowed) {
            this.bPower.setEnabled(true);

        } else {
            //SetEnabled to be false, when it is not allowed
    ...
    public void updateRootAllowed(boolean allowed) {
        ...
        //Set root to be true, when it is allowed
        if (allowed) {
            this.bRoot.setEnabled(true);

        } else {
            //Set root to be false, when it is not allowed
            this.bRoot.setEnabled(false);
